...
class _CheckoutScreenState extends State<CheckoutScreen> {
  int _selectedOrderTab = 0; // 0 for Food, 1 for Instamart
  int _currentStep = 0;
  ...
  void initState() {
    super.initState();
    // Get the order type passed from cart screen
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final args = ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;
      if (args != null && args['orderType'] != null) {
        setState(() {
          _selectedOrderTab = args['orderType'] as int;
        });
      } else {
        // Fallback: Auto-select tab based on which cart has items
  ...
  void _showOrderConfirmation() {
    final orderType = _selectedOrderTab == 0 ? 'Food' : 'Instamart';
    final cart = _selectedOrderTab == 0 
        ? Provider.of<FoodCartProvider>(context, listen: false)
        : Provider.of<InstamartCartProvider>(context, listen: false);
    
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Order Placed!'),
        content: Text('Your $orderType order has been placed successfully. Pay cash on delivery.'),
        actions: [
          TextButton(
            onPressed: () {
              // Clear the cart for the placed order
              cart.clear();
  ...
  Widget build(BuildContext context) {
    ...
    // Determine which cart to show based on selected tab
    final hasFoodItems = foodCart.items.isNotEmpty;
    final hasInstamartItems = instamartCart.items.isNotEmpty;

    return Scaffold(
      backgroundColor: const Color(0xFFF5F5F5),
      appBar: AppBar(
        title: Text(
          '${_selectedOrderTab == 0 ? 'FOOD' : 'INSTAMART'} CHECKOUT',
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                            // Display Cart Items
                            _buildCartItemsCard(
                              _selectedOrderTab == 0 ? foodCart : instamartCart,
                              _selectedOrderTab == 0,
                            ),
      ...
      bottomNavigationBar: !isWeb ? _buildMobileBottomBar(
        _selectedOrderTab == 0 ? foodCart : instamartCart,
        _selectedOrderTab == 0,
      ) : null,
    );
  ...
  Widget _buildCartItemsCard(dynamic cart, bool isFood) {
    return Card(
      color: Colors.white,
      elevation: 0,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Icon(
                    isFood ? Icons.restaurant : Icons.shopping_bag,
                    color: Colors.white,
                    size: 22,
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        isFood ? 'Food Items' : 'Instamart Items',
            ...
            // Cart items list
            ...cart.items.values.map((ci) => Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8),
                border: Border.all(
                  color: const Color(0xFF27A600).withOpacity(0.2),
                ),
              ),
              child: Row(
                children: [
                  Container(
                    ...
                    child: Center(
                      child: Text(
                        ci.item.name.split(' ').first.substring(0, 1).toUpperCase(),
                        style: const TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Text(
                              '₹${ci.item.price.toStringAsFixed(0)} each',
                              style: TextStyle(
                                fontSize: 12,
                                color: Colors.grey.shade600,
                              ),
                  ...
                  Text(
                    '₹${(ci.item.price * ci.qty).toStringAsFixed(0)}',
                    style: const TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: Color(0xFF27A600),
  ...
  Widget _buildOrderSummary(dynamic cart, bool isFood) {
    final subtotal = cart.subtotal;
    final deliveryFee = 20.0;
    final gstCharges = (subtotal + deliveryFee) * 0.05;
    final total = subtotal + deliveryFee + gstCharges;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        isFood ? 'Restaurant Order' : 'Instamart Order',
                        style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                      ),
                      const Text(
                        'Central Bangalore',
            ...
            _buildBillRow('Delivery Fee | 3.0 kms', '₹${deliveryFee.toStringAsFixed(0)}'),
            ...
            Container(
              ...
              child: const Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4),
                  Text(
                    'Note: Please ensure your address and order details are correct.',
                    style: TextStyle(fontSize: 11),
                  ),
                ],
              ),
  ...
  Widget _buildMobileBottomBar(dynamic cart, bool isFood) {
    final subtotal = cart.subtotal;
    final total = (subtotal + 20) * 1.05;
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  '${isFood ? 'Food' : 'Instamart'} Order - TO PAY',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  _currentStep == 2 ? 'PROCEED TO PLACE ORDER' : 'Complete checkout steps',
